...
`MACC rd, rs1, rs2` computes `rd = rd + (rs1 * rs2)` in one cycle.
It's not part of RV32I — it's encoded in the custom-0 opcode space
RISC-V reserves for exactly this kind of extension.

Dot products are otherwise a MUL followed by an ADD per element: two
instructions, two register-file round trips, per multiply-accumulate.
That pattern is the core of most ML inference math (a matrix-vector
multiply is a lot of dot products back to back), which is what MACC
is for — collapsing that pair into one instruction.
...
![MACC path highlighted on the datapath](diagrams/macc_datapath.svg)

(source: [`diagrams/macc_datapath.drawio`](diagrams/macc_datapath.drawio))
